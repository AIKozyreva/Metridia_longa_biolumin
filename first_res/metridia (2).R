...
# Samples from tooths: Ligament. 
# Cell status: undifferentiated, 48 hours of diff, and 10 days after osteogenic differentiation.
...
setwd("/Users/nastyamarkelova/Desktop/metridia_longa/статистика")

count_matrix <- as.matrix(read.csv("./counts_1234567.csv", 
                                   sep = ";", row.names = 1))
head(count_matrix, 2)

#metadata (information about samples)
snames <- colnames(count_matrix)
status <- substr(snames, 12, nchar(snames))
...
summary(res_dif_48h, alpha = 0.05)
...
de <- rownames(res_10d)[1:10]  # if top-10 genes, change to rownames(res_10d)[1:10]
de_mat <- assay(rlt)[de,]
